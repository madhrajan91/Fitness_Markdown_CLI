...
func runSync(cmd *cobra.Command, args []string) {
	...
	// 8. Generate notes (optional, off by default)
	if cfg.EnableObsidian && vaultPath != "" {
		if syncRacesOnly {
		...
		fmt.Println("Obsidian note writing is disabled.")
	}
...
func runImportCache(cmd *cobra.Command, args []string) {
	...
	if err := database.Close(); err == nil {
		...
		// 5. Generate Weekly logs in Obsidian for all history (optional, off by default)
		if cfg.EnableObsidian && vaultPath != "" {
			activitiesByWeek := make(map[string][]*models.MergedActivity)
			for _, ma := range mergedList {
				mon := storage.GetMondayOfWeek(ma.StartTime).Format("2006-01-02")
			...
			fmt.Println("\nObsidian note writing is disabled.")
		}
	}
